...
if(version_compare(JVERSION, '4', '>=')) {
	$jLang->load('com_jmap.sys', JPATH_COMPONENT_ADMINISTRATOR, 'en-GB', true, true);
	JFactory::getApplication()->enqueueMessage (JText::_('COM_JMAP_INSTALLED_VERSION_NOTCOMPATIBLE'), 'error');
}
...
if($format === 'txt' || ($cParams->get('disable_sitemap_formats', 0) && $format && !in_array(0, $disabledSitemapFormats, true) && in_array($format, $disabledSitemapFormats, true))) {
	throw new Exception(JText::_('COM_JMAP_ERROR_LAYOUT_PAGE_NOT_FOUND'), 404);
}
