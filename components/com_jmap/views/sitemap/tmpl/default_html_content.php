...
$this->document->addScriptDeclaration('jmapMergeMenuTreeLevels = "' . $mergeMenuTreeLevels . '";');
...
if (count ( $this->source->data ) != 0) {
	...
	if($containingFolderTitle) {
		echo '<ul data-hash="com_content.container" class="jmap_filetree" style="' . $this->marginSide . '0px"><li><span class="folder">' . JText::_($this->sourceparams->get('title')) . '</span>';
	}
	...
	foreach ( $this->source->data as $elm ) {
		...
		$category = ($isMindMap || !$linkableContentCats) ? $elm->category : '<a target="' . $openTarget . '" href="' . $this->liveSite . JRoute::_ ( ContentHelperRoute::getCategoryRoute($elm->catid, $elm->language ) ) . '">' . $elm->category . '</a>';
