...
<table id="adminListTableFCcategory" class="adminlist">
	...
	<?php foreach ($items as $i => $item) : ?>
		...
		<div id="tablelist_item" class="<?php echo $fc_item_classes.' row'.($i%2 ? 1 : 0); ?>" <?php echo $microdata_itemtype_code; ?>>


		<!-- BOF item fields -->
		<?php foreach ($columns as $name => $label) : ?>

			<div class="fields-block<?php echo $label;?>">
					<a href="<?php echo JRoute::_(FlexicontentHelperRoute::getItemRoute($item->slug, $item->categoryslug, 0, $item)); ?>" itemprop="url">
								<div class="fields_<?php $i++; echo $label;?>"><?php echo isset($item->positions['table']->{$name}->display) ? $item->positions['table']->{$name}->display : ''; ?></div>
					</a>
			</div>
		<?php endforeach; ?>
		<!-- EOF item fields -->
