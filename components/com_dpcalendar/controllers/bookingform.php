...
class DPCalendarControllerBookingForm extends JControllerForm
{
	...
	public function save($key = null, $urlVar = 'b_id')
	{
		...
		$data['processor'] = $this->input->get('paymentmethod');
		...
		if ($result) {
			...
			if ($booking->price > 0 && $booking->id) {
				...
				$this->setRedirect(JRoute::_('index.php?option=com_dpcalendar&view=booking&layout=order&b_id=' . $booking->id . $tmpl, false));
			}
		} else {
			$id = reset(array_keys($data['event_id']));
