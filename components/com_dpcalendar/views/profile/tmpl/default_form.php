	...
	<div class="dp-form__search">
		...
		<button type="button" class="dp-button dp-button-save">
			<?php echo $this->layoutHelper->renderLayout(
				'block.icon',
				['icon' => \DPCalendar\HTML\Block\Icon::SEARCH, 'title' => $this->translate('JSEARCH_FILTER')]
			); ?>
		</button>
		<button type="button" class="dp-button dp-button-clear">
			<?php echo $this->layoutHelper->renderLayout(
				'block.icon',
				['icon' => \DPCalendar\HTML\Block\Icon::DELETE, 'title' => $this->translate('COM_DPCALENDAR_CLEAR')]
			); ?>
		</button>
	</div>
