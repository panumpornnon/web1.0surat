...
class DPCalendarViewMap extends \DPCalendar\View\BaseView
{
	...
	public function init()
	{
		...
		if ($this->input->getInt('module-id')) {
			...
			$params = !empty($menu->params) ? $menu->params : JComponentHelper::getParams('com_dpcalendar');
			$access = !empty($menu->access) ? $menu->access : 1;
		}
		...
		$this->getModel()->setState('filter.location', \DPCalendar\Helper\Location::get($location, false));
