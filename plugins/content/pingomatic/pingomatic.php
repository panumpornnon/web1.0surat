...
class plgContentPingomatic extends JPlugin {
		...
		if (! isset ( $this->context ['routing'] )) {
			...
			if ($articleHelperRoute) {
				$query = "SELECT " .
						 ...
						 "\n WHERE " . $this->db->quoteName('id') . " = " . $this->db->quote($articleHelperRoute);
				$menuCurrentObject = $this->db->setQuery($query)->loadObject();
				if($menuCurrentObject->link == 'index.php?option=com_zoo&view=frontpage&layout=frontpage') {
					...
					$currentAppId = $this->app->input->get('changeapp');
					...
					if($currentAppId != $menuAppId) {
						$query = "SELECT " .
								 ...
								 $this->db->quoteName('params') .
								 "\n FROM " . $this->db->quoteName('#__menu') .
								 "\n WHERE " . $this->db->quoteName('link') . " = " . $this->db->quote('index.php?option=com_zoo&view=frontpage&layout=frontpage');
						$menusZooFrontpage = $this->db->setQuery($query)->loadObjectList();
	...
	public function onContentAfterSave($context, $article, $isNew) {
		...
		if($context == 'com_zoo.item' && ($this->app->input->get('state') != 1 || $articlePublishUpFormatted > $nowFormatted)) {
			return;
		}
				...
				if ($hasArticleMenuRoute) {
						...
						if($this->loadPingerLib()) {
							...
							if ($this->loadCurlLib ()) {
								...
								if($debugEnabled && $indexNowBing->code == 200) {
									$this->app->enqueueMessage ( JText::_( 'COM_JMAP_AUTOPING_DEFAULT_AUTOPING_SENT_INDEXNOW_BING' ), 'notice' );
								}
								...
								if($debugEnabled && $indexNowYandex->code == 202) {
									$this->app->enqueueMessage ( JText::_( 'COM_JMAP_AUTOPING_DEFAULT_AUTOPING_SENT_INDEXNOW_YANDEX' ), 'notice' );
								}
							}
					...
					if($this->cParams->get ( 'enable_google_indexing_api', 0 )) {
						...
						if (PHP_VERSION_ID >= 70205) {
							...
							JModelLegacy::addIncludePath(JPATH_ADMINISTRATOR. '/components/com_jmap/models');
							...
							if($debugEnabled && is_object($apiResponse)) {
								$this->app->enqueueMessage ( JText::sprintf( 'COM_JMAP_GOOGLE_INDEXING_API_SUCCESS', $hasArticleMenuRoute), 'notice' );
							}
						} else {
							if($debugEnabled) {
								$this->app->enqueueMessage ( JText::_('COM_JMAP_PHP_VERSION_UNSUPPORTED_DESC') , 'notice' );
							}
						}
					}
				}
