...
class PlgDPCalendarGoogle extends \DPCalendar\Plugin\SyncPlugin
{
	...
	public function saveEvent($eventId = null, $calendarId, array $data)
	{
		...
		try {
			...
			if (isset($data['location_ids'])) {
				$event->setLocation(\DPCalendar\Helper\Location::format(\DPCalendar\Helper\Location::getLocations((array)$data['location_ids'])));
			}
	...
	public function prepareForm($eventId, $calendarId, $form, $data)
	{
		...
		return parent::cleanupFormForEdit($calendarId, $form, $data);
	}
	...
	public function import()
	{
		...
		if (!$app->input->get('code')) {
			$params = $app->input->get('params', array('client-id' => null, 'client-secret' => null), 'array');
	...
	private function createEventFromGoogle($googleEvent, $calendar, $obj, $colors)
	{
		...
		if (!empty($googleEvent->attachments)) {
			...
			ob_start();
			include $path;
			$event->description .= ob_get_clean();
		}
