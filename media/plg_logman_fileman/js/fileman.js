    ...
    PlgLogmanFileman = {
        Lightbox: {
            init: function (config) {
                ...
                if (!config) config = {};
                if (!config.selector) config.selector = '.fileman-file';
                ...
                $(config.selector).each(function (idx, el) {
                    $(el).click(function (e) {
                        ...
                        var rendered = new EJS({element: document.getElementById('fileman-file-template')}).render(data),
                            element = $('<div class="k-ui-namespace fileman-file-modal"></div>').append(rendered),
                            container = $('#fileman-file-tmp'),
