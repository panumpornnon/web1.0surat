...
!function($, Wf) {
    ...
    function arraysEqual(arr1, arr2) {
        if (arr1.length === arr2.length) {
            for (let i = 0; i < arr1.length; i++) if (arr1[i] !== arr2[i]) return;
            return 1;
        }
    }
    ...
    function createPreviewThumbnails() {
        $("#item-list .file").filter(".jpg,.jpeg,.bmp,.tiff,.png,.webp,.gif").not(".thumbnail-loading,.thumbnail-loaded,.thumbnail-error").each(function() {
            var item, img, src;
            src = $(item = this).data("preview"), (src = $(item).data("thumbnail") ? $(item).data("thumbnail") : src) && (img = $('<img class="uk-thumbnail uk-position-cover" loading="lazy" />'), 
    ...
    function togglePreviewThumbnails() {
        "grid" === getMode() ? (createPreviewThumbnails(), $("#item-list").data("sortable") && $("#item-list").sortable("option", "axis", !1)) : ($("#item-list").off("click.item-list-images"), 
        $("#item-list").data("sortable") && $("#item-list").sortable("option", "axis", "y"));
    }
    function getMode() {
    ...
    function toggleMode(mode) {
        $("#browser").toggleClass("view-mode-grid", "grid" === mode), togglePreviewThumbnails();
    }
    function editImage() {
        var items = $.fn.filebrowser.getselected();
        if (1 < items.length || isMobile()) return function(items) {
                ...
                onOpen: function() {
                    ...
                    if (options.upload_resize && options.can_edit_images) {
                        ...
                        for (var num = Math.max(rw.length, rh.length), i = 0; i < num; i++) {
                            var values, found, $opt = $resize.clone(), w = rw[i] || "", h = rh[i] || "";
                            1 < num && !w && !h || (found = !(values = [ w, h ]), 
                            $("#resize-dialog").find(".uk-repeatable").each(function() {
                                if (arraysEqual($(this).data("values"), values)) return found = !0;
                            }), found) || ($opt.data("values", values), $("#resize-dialog").append($opt), 
                            ...
                            $opt.find(".uk-constrain-checkbox").constrain(), $opt.repeatable().on("repeatable:create", function(e, o, n) {
                                ...
                                $(elm).remove();
                            }), w = parseInt(options.upload_resize_crop, 10), $opt.find('input[name^="resize_crop"]').prop("checked", !!w).on("click", function() {
        ...
        var items = items[0], ed = tinyMCEPopup.editor, iw = parseFloat($(items).data("width")), ih = parseFloat($(items).data("height")), src = $(items).attr("id"), items = $(items).data("preview"), query = items.indexOf("?"), query = (-1 !== query && (items = items.substring(0, query)), 
        $.extend(ed.windowManager.params, {
            src: src,
            url: items,
            width: iw,
            height: ih,
    ...
    function editText() {
        var item = $.fn.filebrowser.getselected(), ed = tinyMCEPopup.editor, src = $(item).attr("id"), item = $(item).data("preview"), query = item.indexOf("?"), query = (-1 !== query && (item = item.substring(0, query)), 
        $.extend(ed.windowManager.params, {
            src: src,
            url: item,
            onsave: function(item) {
    ...
    function getUploadOptions() {
        $("#upload-options").append(function() {
            var layout = [];
            if (!options.upload_resize || !options.can_edit_images) return [];
            for (var rl = options.upload_resize_label || "", rw = options.upload_resize_width || "", rh = options.upload_resize_height || "", rs = options.upload_resize_suffix || "", crop = options.upload_resize_crop || "", state = parseInt(options.upload_resize_state, 10), $resizeOpt = $(`
        <div class="uk-form-row uk-form-striped uk-flex uk-width-1-1 uk-position-relative uk-repeatable uk-margin-small">
            <div class="uk-width-1-1 uk-flex uk-flex-wrap">

                <div class="uk-width-1-1 uk-width-medium-1-6 uk-flex uk-flex-middle">
                    <label>
                        <input name="upload_resize_item_state[]" type="checkbox" value="1"
                            class="uk-margin-small-right" checked />
                    </label>
                    <span class="uk-form-label uk-width-1-1 uk-width-medium-1-10"></span>
                </div>

                <div class="uk-flex uk-flex-wrap uk-width-1-1 uk-width-small-1-1 uk-width-medium-3-5 uk-form-constrain">
                    <div class="uk-flex uk-width-1-1 uk-width-medium-2-3">
                
                        <div class="uk-form-controls uk-width-small-1-6">
                            <input type="text" name="upload_resize_width[]" class="uk-text-center" value="" />
                        </div>

                        <div class="uk-form-controls uk-margin-small-left uk-margin-small-right">
                            <strong class="uk-text-center uk-vertical-align-middle uk-display-block">&times;</strong>
                        </div>

                        <div class="uk-form-controls uk-width-small-1-6">
                            <input type="text" name="upload_resize_height[]" class="uk-text-center" value="" />
                        </div>

                        <div class="uk-form-controls uk-width-medium-1-3 uk-width-small-1-6">
                            <label class="uk-form-label uk-constrain-label"
                                title="${tinyMCEPopup.getLang("dlg.proportional", "Proportional")}">
                                <input class="uk-constrain-checkbox" type="checkbox" checked />
                                ${tinyMCEPopup.getLang("dlg.proportional", "Proportional")}
                            </label>
                        </div>

                    </div>

                    <div class="uk-form-controls uk-width-1-1 uk-width-medium-1-4">
                        <label class="uk-form-label uk-width-1-1"
                            title="${tinyMCEPopup.getLang("dlg.upload_crop", "Crop to Fit")}">
                            <input name="upload_resize_crop[]" type="checkbox" value="0"
                                class="uk-margin-small-right" />
                            ${tinyMCEPopup.getLang("dlg.upload_crop", "Crop to Fit")}
                        </label>
                    </div>
                </div>

                <div class="uk-form-controls uk-width-1-1 uk-width-medium-1-6">
                    <label class="uk-form-label uk-width-1-5">
                        ${tinyMCEPopup.getLang("dlg.upload_resize_suffix", "Suffix")}
                    </label>
                    <div class="uk-form-controls uk-width-1-1">
                        <input type="text" name="upload_resize_suffix[]" value="" class="uk-margin-small-left" />
                    </div>
                </div>
            ...
            <div class="uk-position-right">
                <button class="uk-button uk-button-link uk-repeatable-create">
                    <i class="uk-icon-plus"></i>
                </button>
                <button class="uk-button uk-button-link uk-repeatable-delete">
                    <i class="uk-icon-trash"></i>
                </button>
            </div>
        </div>
        `), $resize = $(`
        <div class="uk-width-1-1 uk-flex uk-flex-wrap uk-placeholder uk-placeholder-small">
            <div class="uk-margin-remove uk-width-1-1 uk-flex">
                <div class="uk-width-1-1 uk-margin-small-left">
                    <input name="upload_resize_state"
                        type="checkbox"
                        value="0"
                        class="uk-margin-small-right" />
                    <label for="upload_resize_state"
                        class="hastip"
                        title="${tinyMCEPopup.getLang("dlg.upload_resize_tip", "Resize Image")}">
                        ${tinyMCEPopup.getLang("dlg.resize", "Resize Image")}
                    </label>
                </div>
                <div class="uk-position-right">
                    <button class="uk-button uk-button-link uk-margin-small-right uk-toggle" data-target="upload_resize_options" title="${tinyMCEPopup.getLang("dlg.upload_toggle_options", "Toggle Options")}">
                        <svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 512 512">
                            <path d="M64 416l192-320 192 320z"></path>
                            <path d="M448 96l-192 320-192-320z"></path>
                        </svg>
                    </button>
                </div>
            </div>
        </div>
        `), num = ("string" == typeof rw && (rw = rw.split(",")), "string" == typeof rh && (rh = rh.split(",")), 
            "string" == typeof rl && (rl = rl.split(",")), "string" == typeof rs && (rs = rs.split(",")), 
            "string" == typeof crop && (crop = crop.split(",")), Math.max(rw.length, rh.length)), i = 0; i < num; i++) {
                var values, found, $opt = $resizeOpt.clone(), w = rw[i] || "", h = rh[i] || "", sfx = rs[i] || "", lbl = rl[i] || "", ctf = crop[i] || "";
                1 < num && !w && !h || (found = !(values = [ w, h, lbl, sfx, ctf ]), 
                $.each(layout, function(i, $el) {
                    if (arraysEqual($el.data("values"), values)) return found = !0;
                }), found) || ($opt.data("values", values), layout.push($opt), $opt.find("span").text(lbl), 
                ...
                $opt.repeatable().on("repeatable:create", function(e, o, n) {
                    ...
                    $(elm).remove();
                }), lbl = parseInt(ctf, 10), $opt.find('input[name^="upload_resize_crop"]').prop("checked", !!lbl).on("click", function() {
                    this.value = this.checked ? 1 : 0;
                }).val(lbl), $opt.find('input[name^="upload_resize_item_state"]').on("update", function() {
            ...
            var $container = $('<div class="uk-flex uk-flex-column uk-width-1-1" id="upload_resize_options"/>').append(layout);
            return $resize.append($container), $resize.find('input[name="upload_resize_state"]').prop("checked", !!state).on("click", function() {
                var el = this;
                this.value = this.checked ? 1 : 0, $(".uk-repeatable", "#upload-options").each(function() {
                    $(this).find(":input").not(el).prop("disabled", !el.checked);
                }), this.checked && $resize.parent().find('input[name^="upload_resize_item_state"]').trigger("update");
            }).val(state), $resize.find(".uk-toggle").on("click", function(e) {
                e.preventDefault();
                e = $(this).attr("data-target");
                $(this).hasClass("uk-toggle-active") ? $resize.find("#" + e).show() : $resize.find("#" + e).hide(), 
                $(this).toggleClass("uk-toggle-active");
            }), $resize.find(":input").not('input[name="upload_resize_state"], button.uk-toggle').prop("disabled", !state), 
            $resize;
        }());
        var th, tw, $watermark = $(`
            <div class="uk-form-row uk-flex uk-flex-wrap uk-width-1-1 uk-placeholder uk-placeholder-small" id="upload_watermark_options">
                <div class="uk-width-1-1 uk-width-medium-1-6 uk-margin-small-left">
                    <input id="upload_watermark" name="upload_watermark_state" type="checkbox" value="0" class="uk-margin-small-right" />
                    <label for="upload_watermark" title="${tinyMCEPopup.getLang("dlg.upload_watermark_tip", "Watermark Image")}" class="hastip">${tinyMCEPopup.getLang("dlg.upload_watermark", "Watermark Image")}</label>
                </div>
            </div>
        `), $thumbnail = $(`
        <div class="uk-form-row uk-flex uk-flex-wrap uk-width-1-1 uk-placeholder uk-placeholder-small">

            <div class="uk-width-1-1 uk-margin-small-left">
                <input name="upload_thumbnail_state"
                    type="checkbox"
                    value="0"
                    class="uk-margin-small-right" />

                <label for="upload_thumbnail_state"
                    class="hastip"
                    title="${tinyMCEPopup.getLang("dlg.upload_thumbnail_tip", "Thumbnail")}">
                    ${tinyMCEPopup.getLang("dlg.upload_thumbnail", "Thumbnail")}
                </label>
            </div>

            <div class="uk-width-1-1 uk-flex">

                <div class="uk-width-1-1 uk-width-medium-2-5 uk-width-small-3-5 uk-form-constrain uk-flex uk-flex-wrap">

                    <div class="uk-form-controls uk-width-1-4 uk-width-medium-1-6">
                        <input type="text"
                    ...
                    <div class="uk-form-controls uk-width-auto uk-width-medium-3-6">
                        <label class="uk-form-label uk-width-1-1 uk-constrain-label"
                            title="${tinyMCEPopup.getLang("dlg.proportional", "Proportional")}">
                            <input class="uk-constrain-checkbox" type="checkbox" checked />
                            ${tinyMCEPopup.getLang("dlg.proportional", "Proportional")}
                        </label>
                ...
                <div class="uk-width-1-1 uk-width-medium-3-5 uk-width-small-2-5">
                    <label class="uk-form-label uk-width-1-1 hastip"
                        title="${tinyMCEPopup.getLang("dlg.upload_crop", "Crop to Fit")}">
                        <input class="uk-margin-right"
                            name="upload_thumbnail_crop"
                            type="checkbox"
                            value="0" />
                        ${tinyMCEPopup.getLang("dlg.upload_crop", "Crop Thumbnail")}
                    </label>
                </div>
        ...
        $("#upload-options").append($watermark).find('input[name="upload_watermark_state"]').prop("checked", !!tw).on("click", function() {
            this.value = this.checked ? 1 : 0;
        }).val(tw)), (options.upload_resize || options.upload_watermark) && options.can_edit_images && ($("#upload-options").wrap("<fieldset />").parent().prepend('<legend class="uk-text-bold">' + tinyMCEPopup.getLang("dlg.image_options", "Image Options") + "</legend>"), 
        ...
        $("#upload-queue").on("uploadwidget:fileadded", function(e, file) {
            var $opt;
            /^(jpg|jpeg|png|bmp|tiff|gif)$/i.test(file.extension) && options.upload_resize && options.can_edit_images && (($opt = $(`
                <div class="uk-grid uk-grid-collapse uk-width-1-1 queue-item-resize uk-placeholder uk-placeholder-small uk-repeatable uk-position-relative uk-hidden">

                    <div class="uk-width-1-10">
                        <label for="upload_resize"
                            class="uk-form-label"
                            title="${tinyMCEPopup.getLang("dlg.upload_resize_tip", "Resize")}">
                            ${tinyMCEPopup.getLang("dlg.resize", "Resize")}
                        </label>
                    </div>

                    <div class="uk-width-1-1 uk-width-medium-4-5 uk-flex uk-flex-wrap">

                        <div class="uk-width-1-1 uk-width-small-3-5 uk-width-medium-2-5 uk-form-constrain uk-flex uk-margin-small-top">

                            <div class="uk-form-controls uk-width-1-4 uk-width-medium-1-6">
                                <input type="text"
                                    name="upload_file_resize_width[]"
                                    class="uk-text-center"
                                    value="" />
                            </div>

                            <div class="uk-form-controls">
                                <strong class="uk-form-label uk-text-center uk-vertical-align-middle uk-display-block">&times;</strong>
                            </div>

                            <div class="uk-form-controls uk-width-1-4 uk-width-medium-1-6">
                                <input type="text"
                                    name="upload_file_resize_height[]"
                                    class="uk-text-center"
                                    value="" />
                            </div>
                        ...
                        <div class="uk-width-1-1 uk-width-medium-1-5 uk-flex uk-margin-small-top">
                            <label class="uk-form-label uk-width-1-1"
                                title="${tinyMCEPopup.getLang("dlg.upload_crop", "Crop to Fit")}">
                                <input class="uk-margin-right"
                                    name="upload_file_resize_crop[]"
                                    type="checkbox"
                                    value="0" />
                                ${tinyMCEPopup.getLang("dlg.upload_crop", "Crop to Fit")}
                            </label>
                        ...
                        <div class="uk-width-medium-1-5 uk-flex uk-margin-small-top">
                            <label class="uk-form-label uk-width-1-5">
                                ${tinyMCEPopup.getLang("dlg.upload_resize_suffix", "Suffix")}
                            </label>
                            <div class="uk-form-controls uk-width-4-5 uk-margin-small-right">
                                <input type="text"
                                    name="upload_file_resize_suffix[]"
                    ...
                    <div class="uk-text-right uk-position-top-right">
                        <button class="uk-button uk-button-link uk-repeatable-create uk-margin-small-top">
                            <i class="uk-icon-plus"></i>
                        </button>
                        <button class="uk-button uk-button-link uk-repeatable-delete uk-margin-small-top">
                            <i class="uk-icon-trash"></i>
                        </button>
                    </div>

                </div>
                `)).appendTo(file.element).repeatable().on("repeatable:create", function(e, o, n) {
                $(n).find(".uk-constrain-checkbox").trigger("constrain:update");
            }).on("repeatable:delete", function(e, ctrl, elm) {
                $(elm).remove();
    ...
    function createThumbnail() {
        ...
        options.thumbnail_height = 90), 1 < items.length || isMobile()) return function(items) {
            Wf.Modal.dialog(tinyMCEPopup.getLang("dlg.create_thumbnail", "Create Thumbnail"), "", {
                ...
                <div class="uk-form-row uk-grid uk-grid-small">
                    <label class="uk-form-label uk-width-3-10"
                        title="${tinyMCEPopup.getLang("dlg.dimensions", "Dimensions")}">
                        ${tinyMCEPopup.getLang("dlg.dimensions", "Dimensions")}
                    </label>

                    <div class="uk-width-2-3 uk-form-constrain uk-flex">
                ...
                <div class="uk-form-row uk-grid uk-grid-small">
                    <label for="thumbnail_quality"
                        class="uk-form-label uk-width-3-10">
                        ${tinyMCEPopup.getLang("dlg.quality", "Quality")}
                    </label>
                ...
                `, html = ($("#thumbnail-create-dialog").append(html), options.upload_thumbnail_width || options.upload_thumbnail_height || (options.upload_thumbnail_width = 120, 
                    ...
                    parseInt(options.upload_thumbnail_crop, 10));
                    $("#thumbnail-create-dialog").find('input[name="thumbnail_crop"]').prop("checked", !!html).on("click", function() {
                        this.value = this.checked ? 1 : 0;
                    }).val(html);
                },
                buttons: [ {
