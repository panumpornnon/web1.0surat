...
!function() {
    ...
    tinymce.PluginManager.add("source", function(ed, url) {
        ...
        this.cursorPos = 0, this.setContent = function(v) {
            ...
            var el = ed.getElement(), args = {
                content: content = tinymce.is(content) ? content : this.getContent(),
                no_events: !0,
                format: "raw"
            };
            return !1 !== ed.settings.source_validate_content && (ed.setContent(content, {}), 
            args.content = ed.getContent()), /TEXTAREA|INPUT/i.test(el.nodeName) ? el.value = args.content : el.innerHTML = args.content, 
            debounced && ed.onWfEditorChange.dispatch(ed, args), args.content;
