...
<!-- Calendly inline widget end -->`);
    ...
    function insertMedia(ed, data, provider) {
        ...
        data.html && (data.html = Calendly.toIframe(ed, data.html), function(html) {
            var div;
            if (html && "string" == typeof html) return (div = document.createElement("div")).innerHTML = html, 
            (html = div.querySelector("iframe, video, audio")) && !!html.closest('div[style*="position:relative"][style*="padding-bottom:56.25%"]');
        }(data.html) && (data.responsive = !0), attribs = function(html) {
            var attribs = {
                html: ""
    ...
    function getMediaData(ed, value) {
        ...
        return value.responsive && (data.responsive = !0, delete value.responsive), 
        value.html && (data.html = value.html, delete value.html), each(value, function(val, name) {
            return -1 !== tinymce.inArray(invalidAttributes, name) || "" == val && !boolAttrs[name] || ("attributes" == name ? (each(val, function(attrValue, attrName) {
    ...
    function getMediaHtml(ed, value) {
        value = getMediaData(ed, value), ed = ed.dom.createHTML(value.nodeName, value.attributes, value.html);
        return ed = value.responsive ? '<div style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;">' + ed + "</div>" : ed;
    }
