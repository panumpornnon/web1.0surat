...
!function($) {
    ...
    function updateMediaUrl(row, options) {
        $(row).find(".field-media-wrapper").add(row).each(function() {
            ...
            $inp.hasClass("wf-media-input-converted")) {
                ...
                var extensions, name = $inp.attr("name") || "", hasScheme = params.path && -1 !== params.path.indexOf(":");
                name && (name = name.replace(/jform\[com_fields\]\[([\w-]+)\].*/, "$1"), 
                name = options.mediafields[name] || {}, hasScheme ? (hasScheme = params.path.split(":")[0], 
                name.directory && (params.path = hasScheme + ":" + name.directory.replace(/^\/+/, ""))) : (name.directory && (params.folder = name.directory.replace(/^\/+/, "")), 
                delete params.path));
            }
            hasScheme = getBasePath($inp) + "index.php?option=com_jce&task=mediafield.display&plugin=" + plugin + "&fieldid=" + id + "&mediatype=" + dataUrl;
            options.context && (params.context = options.context);
            $.each([ "option", "task", "plugin", "fieldid", "mediatype", "element", "view", "tmpl" ], function(i, key) {
                delete params[key];
            }), hasScheme += "&" + $.param(params), this.hasAttribute("data-url") && this.setAttribute("data-url", hasScheme), 
            $(this).is("joomla-field-media, .wf-media-wrapper-custom") && (this.setAttribute("url", hasScheme), 
            name = Joomla.sanitizeHtml('<iframe src="' + hasScheme + '" class="iframe" title="" width="100%" height="100%"></iframe>', {
                iframe: [ "src", "class", "title", "width", "height" ]
            }), $(this).find(".joomla-modal").attr("data-url", hasScheme).attr("data-iframe", name), 
            $(this).removeAttr("modal-width").removeAttr("modal-height")), $linkBtn.length && $linkBtn.attr("href", hasScheme);
        });
    }
    ...
    $.fn.WfMediaUpload = function() {
        return this.each(function() {
            ...
            $('<label title="Upload" role="button" class="btn btn-outline-secondary wf-media-upload-button" aria-label="Upload"><i role="presentation" class="icon-upload"></i><input type="file" aria-hidden="true" /></label>'));
