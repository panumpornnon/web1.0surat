...
class CoalawebtrafficHelperIptools
{
    ...
    private static function _real_getUserIP() {
        //Should we check for proxies?
        $comParams = JComponentHelper::getParams('com_coalawebtraffic');
        $checkProxy = $comParams->get('check_proxy', 1);

        // Normally the $_SERVER superglobal is set
        if (isset($_SERVER)) {

            //If we need to check what is contained in SERVER we can turn on the option below
            //and save it to a log file for further investigation
            if ($comParams->get('log_debug')) {
                //Start our log file code
                JLog::addLogger(array('text_file' => 'com_coalawebtraffic_debug.log.php'), JLog::DEBUG, 'com_coalawebtraffic_debug');
                ...
                //Save our json encoded array
                JLog::add(json_encode($logit), JLog::DEBUG, 'com_coalawebtraffic_debug');
            }
            ...
            if ($checkProxy) {
                ...
                foreach ($ip_keys as $key) {
                    ...
                    if (array_key_exists($key, $_SERVER) === true) {
                        foreach (explode(',', $_SERVER[$key]) as $ip) {
                            ...
                            JLog::add($ip, JLog::WARNING, 'com_coalawebtraffic');
                            $ip = trim($ip);
                ...
                //This is done to over come strange behaviour by OVH hosting servers
                $ip = strtok($_SERVER['REMOTE_ADDR'], " ");
                $ip = trim($ip);
                // attempt to validate IP
                if (self::validate_ip($ip)) {
                    return $ip;
