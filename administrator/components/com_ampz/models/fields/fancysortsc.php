...
class JFormFieldRH_FancySortSc extends JFormField
{
    protected $type = 'FancySortSc';
    ...
    protected function getInput()
    {
        ...
        foreach ($options as $i => $option)
        {
            ...
            $followShareOnly = $option->followShareOnly;
            $followChecked = "";
            if ($followShareOnly == "share") {
                $followShareOnly = "disabled";
            }
            else if ($followShareOnly == "follow") {
                $followChecked = "checked ";
                $followShareOnly = "readonly";
                $onclick = ' onclick="return false"';
            }

            $follow = strpos($enabled_networks, $network . '_follow');
            if ($follow !== false) {
                $followChecked =  "checked ";
            }

            // Check if the network is enabled
            $pos = strpos($enabled_networks, 'btn_' . $network);
            if ($pos === false)
            {
                $checked = "";

                $temp_unchecked[] = '<li class="sortable-item '. $network .'" data-id="'. $network .'"><div class="sortable-handle"></div><div class="sortable-content '.  $network.'"><span class="sortable-content-text">'. $labels .'</span><input type="checkbox" id="btn_' . $option->network . '" name="' . $this->name . '" value="' . htmlspecialchars($option->value, ENT_COMPAT, 'UTF-8') . '"' . $checked . $class . $labels . $onchange . $disabled . '/><input type="checkbox" id="btn_share_follow_' . $option->network . '" name="' . $this->name . '" class="labelauty_small_share_follow" value="' . $option->network . '_follow"' . $followChecked . $onclick . $onchange . $followShareOnly . '/></div></li>';
            }
            else
            {
                $checked = "checked";

                $order_start_pos = $pos + strlen('btn_' . $network) + 1;
                ...
                $temp_checked[$order] = '<li class="sortable-item '. $network .'" data-id="'. $network .'"><div class="sortable-handle"></div><div class="sortable-content '.  $network.'"><span class="sortable-content-text">'. $labels .'</span><input type="checkbox" id="btn_' . $option->network . '" name="' . $this->name . '" value="' . $value . '"' . $checked . $class . $labels . $onchange . $disabled . '/><input type="checkbox" id="btn_share_follow_' . $option->network . '" name="' . $this->name . '" class="labelauty_small_share_follow" value="' . $option->network . '_follow"' . $followChecked . $onclick . $onchange . $followShareOnly . '/></div></li>';
            }
    ...
    protected function getOptions()
    {
        ...
        foreach ($this->element->children() as $option)
        {
            ...
            $followShareOnly = (string) $option['followShareOnly'];
            ...
            $tmp->checked = $checked;
            $tmp->followShareOnly = $followShareOnly;
            $tmp->labels = $labels;
