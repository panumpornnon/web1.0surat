    ...
    <?php foreach ($items as $i => $item) :
        ...
        <div <?= $filter ?>>
            <div class="<?= $panel[$i % 2] ?><?= $panel_hover ?>">
                ...
                <div class="<?= $card ? '{wk}-card-badge' : '{wk}-position-z-index {wk}-position-top-right {wk}-position-medium' ?> <?= $badge_style ?>"><?= $item['badge'] ?></div>
                <?php endif ?>

                <?php if ($item['media'] && $settings['media'] && $settings['media_align'] == 'teaser') : ?>
                <div class="{wk}-text-center <?= $card && $settings['media_align'] == 'teaser' ? '{wk}-card-media-top' : '{wk}-margin {wk}-margin-remove-top' ?>"><?= $media ?></div>
                <?php endif ?>
