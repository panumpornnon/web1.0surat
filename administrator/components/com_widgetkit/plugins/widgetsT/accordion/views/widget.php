...
// JS Options
$options = array();
$options[] = $settings['collapse'] ? '' : 'multiple: true';
...
<ul class="{wk}-accordion {wk}-text-<?= $settings['text_align'] ?> <?= $settings['class'] ?>" {wk}-accordion="<?= $options ?>">

    <?php foreach ($items as $i => $item) : ?>
    <li <?= ($i == 0 && $settings['first_item']) ? 'class="{wk}-open"' : '' ?>>
    ...
    ?>
        ...
        <a href class="{wk}-accordion-title"><?= $item['title'] ?></a>
