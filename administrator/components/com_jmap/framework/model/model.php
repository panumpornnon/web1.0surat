...
class JMapModel extends JModelLegacy implements IJMapModel {
	...
	protected function storeActionLog($action, $context, $contextDescription, $link, $itemId) {
		...
		$actionLogTable->message_language_key = JText::sprintf('COM_JMAP_ACTIONLOGS_STRING', $action, $contextDescription);
		$actionLogTable->message = json_encode($messageObject);
		$actionLogTable->log_date = (string)JFactory::getDate();
		$actionLogTable->extension = 'com_jmap.' . $context;
		$actionLogTable->user_id = $user->id;
		$actionLogTable->item_id = $itemId;
		
		$params = JComponentHelper::getComponent('com_actionlogs')->getParams();
		...
		try {
			$this->_db->insertObject('#__action_logs', $actionLogTable);
