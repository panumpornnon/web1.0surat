	...
	*/
	...
	function update($parent) {
		...
		try {
			...
			echo '<div style="width:fit-content;margin:10px 0 10px 12px" class="alert alert-info"><span class="icon-info"></span>' . JText::_('COM_JMAP_CLEAR_BROWSER_CACHE') . '</div>';
		} catch (Exception $e) {
	...
	function postflight($type, $parent) { 
		...
		if($type == 'update' && $this->getParam('seostats_service') == 'alexa') {
			...
			$fileToDelete = JPATH_ROOT . '/administrator/components/com_jmap/views/cpanel/tmpl/default_alexa.php';
