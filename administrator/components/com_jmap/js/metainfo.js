				...
				promisesCollection[index].then(function(responseData, status) {
					...
					if(jmap_metainfoAutopopulateSocialimageSelector) {
						...
						if(socialImage && !$(targetSocialImage).val()) {
							...
							// Refresh the image popover
							var imgPreview = new Image(200, 200);
							imgPreview.src = jmap_baseURI + socialImage;
							var containerPreview = $(targetSocialImage).prev('span.field-media-preview');
							try {
								containerPreview.popover('destroy');
							} catch(err) {}
							containerPreview.popover({content: imgPreview});
