...
jQuery(function($){
	var CPanel = $.newClass ({
		 ...
		 * @var String
		 */
		writingProgress : '<div class="progress progress-striped active">' +
							'<div id="progressBarWriting" class="progress-bar" role="progressbar" aria-valuenow="" aria-valuemin="0" aria-valuemax="100">' +
								'<span class="sr-only"></span>' +
							'</div>' +
						'</div>',
    	/**
		...
		registerEvents : function() {
			...
			$('input.hasClickPopover').popover({
				...
				content: function() {
					...
					var concatenatePingXmlFormat = 	"<a data-role='pinger' class='pinger glyphicon glyphicon-flash' href='https://www.google.com/ping?sitemap=" + encodeURIComponent(jmap_livesite + '/' + sitemapFrontendFilename) + "'>" + COM_JMAP_PING_GOOGLE + "</a>" +
		 											"<a data-role='pinger' data-type='rpc' class='pinger glyphicon glyphicon-flash' href='https://www.bing.com/indexnow?key=28bcb027f9b443719ceac7cd30556c3c&url=" + encodeURIComponent(jmap_livesite + '/' + sitemapFrontendFilename) + "'>" + COM_JMAP_PING_BING + "</a>" +
													...
													"<a data-role='pinger' data-type='rpc' class='pinger glyphicon glyphicon-flash' href='https://www.baidu.com/s?wd=" + encodeURIComponent(jmap_livesite + '/' + sitemapFrontendFilename) + "'>" + COM_JMAP_PING_BAIDU + "</a>";
			...
			if(navigator.clipboard) {
				$(document).on('click', '#jmap_seo div.single_container.xmlcontainer label.label-primary,#xmlsitemap_export label.label-primary,#rssfeed label.label-primary', function(jqEvent){
					navigator.clipboard.writeText($(jqEvent.target).nextAll('input.sitemap_links').val())
					.then(function() {
						$(jqEvent.target).tooltip({
							trigger : 'click', 
							placement : 'right',
						...
						setTimeout(function(){
							$(jqEvent.target).tooltip('destroy');
						}, 2000);
					})
					.catch(function(err) {
					});
		...
		writeSitemapFile : function(element) {
			...
			// Build modal dialog
			var modalDialog =	'<div class="modal fade" id="writingModal">' +
									'<div class="modal-dialog">' +
										'<div class="modal-content">' +
											'<div class="modal-header">' +
								        		'<h4 class="modal-title">' + COM_JMAP_WRITE_XML_WRITE + '</h4>' +
								        		'<label class="closewriting glyphicon glyphicon-remove-circle"></label>' +
							        		'</div>' +
							        		'<div class="modal-body">' +
			...
			$('#writingModal').on('shown.bs.modal', function(event) {
				...
				$.ajax(sitemapLink + '&cronjobclient=1', {
					...
					if(data) {
						...
						$('#progressBarWriting').css({'width':'100%'}).addClass('progress-bar-success');
						// Inform user process initializing
						...
						$('#progressBarWriting').parent('div.progress').removeClass('progress-striped active');
					} else {
						// Phase 1 KO, stop the process with error here and don't go on
						$('#progressBarWriting').css({'width':'100%'}).addClass('progress-bar-danger');
						// Append exit message
			...
			$('#writingModal').modal(modalOptions);
		...
		rpcSitemapPing : function(element) {
			...
			$('#pingingModal').on('shown.bs.modal', function(event) {
				...
				$.ajax("../administrator/index.php?option=com_jmap&task=ajaxserver.display&format=json", {
					...
					if(data.result) {
						$('#progressBar1').css({'width':'100%'}).addClass('progress-bar-success');
						$('#progressBar1').parent('div.progress').removeClass('progress-striped active');
						// Inform user process initializing
