		...
		try {
			...
			if($this->getState('inspectlink')) {
				...
				if (PHP_VERSION_ID >= 70205) {
					...
					$jsonResponseAsArray = json_encode($service->sitemaps->inspect($inspectionUrl, $searchConsoleDomain));
					$results['inspect'] = json_decode($jsonResponseAsArray);
				}
			}
		...
		if (! $result = $this->client->getAccessToken ()) { // auth call to google
			...
			if ($params->get('google_indexing_authcode') && !$params->get('google_indexing_authtoken')) {
				...
				if($googleAccessToken) {
					$configModel = JModelLegacy::getInstance('Config', 'JMapModel');
					...
					$this->app->enqueueMessage ( JText::_('COM_JMAP_GOOGLE_AUTHENTICATION_ENABLED') );
					$this->app->redirect( 'index.php?option=com_jmap&task=config.display', JText::_('COM_JMAP_SAVED_PARAMS'));
				} else {
