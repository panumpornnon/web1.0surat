...
class JMapModelSources extends JMapModel implements IJMapModelSources {
						...
						if(strpos($chunksObject->{'where'.$wjt.'_value_jointable'.$jt}, ',')) {
							...
							if(preg_match('/\{\d+months\}/', $chunksObject->{'where'.$wjt.'_value_jointable'.$jt})) {
								...
								if($jtoperator === ' LIKE ') {
									...
									if(JString::strtoupper($chunksObject->{'where'.$wjt.'_value_jointable'.$jt}) === 'NULL') {
										$isJtCondition = $jtoperator === ' = ' ? ' IS ' : ' IS NOT ';
										$whereConditions[] = $this->_db->quoteName($joinTable) . "." . $this->_db->quoteName($chunksObject->{'where'.$wjt.'_jointable'.$jt}) . $isJtCondition . $chunksObject->{'where'.$wjt.'_value_jointable'.$jt};
									} else {
										$whereConditions[] = $this->_db->quoteName($joinTable) . "." . $this->_db->quoteName($chunksObject->{'where'.$wjt.'_jointable'.$jt}) . $jtoperator . $this->_db->quote($chunksObject->{'where'.$wjt.'_value_jointable'.$jt});
									}
								}
			...
			if(is_array($tableFields) && count($tableFields)) {
				...
				if($paramsRegistry->get('created_date', null) && in_array('created', $tableFields)) {
					...
					if($mainTable == '#__jevents_vevdetail') {
						$whereConditions[] = $this->_db->quoteName('#__jevents_vevent') . "." . $this->_db->quoteName('created') . " > {" . $latestMonths . "months}" ;
					} else {
					...
					if(strpos($chunksObject->{'where'.$wmt.'_value_maintable'}, ',')) {
						...
						if(preg_match('/\{\d+months\}/', $chunksObject->{'where'.$wmt.'_value_maintable'})) {
							...
							if($operator === ' LIKE ') {
								...
								if(JString::strtoupper($chunksObject->{'where'.$wmt.'_value_maintable'}) === 'NULL') {
									$isCondition = $operator === ' = ' ? ' IS ' : ' IS NOT ';
									$whereConditions[] = $this->_db->quoteName($mainTable) . "." . $this->_db->quoteName($chunksObject->{'where'.$wmt.'_maintable'}) . $isCondition . $chunksObject->{'where'.$wmt.'_value_maintable'};
								} else {
									$whereConditions[] = $this->_db->quoteName($mainTable) . "." . $this->_db->quoteName($chunksObject->{'where'.$wmt.'_maintable'}) . $operator . $this->_db->quote($chunksObject->{'where'.$wmt.'_value_maintable'});
								}
							}
	...
	public function storeEntity($forceRegenerate = false, $wizard = false, $wizardModel = null) {
		...
		if(version_compare(JVERSION, '3.9', '>=') && $this->getComponentParams()->get('actionlogs_integration', 0)) {
			$isNew = $this->requestArray['id'] ? false : true;
			$action = $isNew ? JText::_('COM_JMAP_ACTIONLOGS_ADDED') : JText::_('COM_JMAP_ACTIONLOGS_UPDATED');
			$this->storeActionLog($action, 'sources', JText::sprintf('COM_JMAP_ACTIONLOGS_DATA_SOURCE', $table->name), 'index.php?option=com_jmap&task=sources.editEntity&cid[]=' . $table->id, $table->id);
		}
		return $table;
	}
	...
	public function copyEntity($ids) { 
		...
		// If integration with ActionLogs
		if(version_compare(JVERSION, '3.9', '>=') && $this->getComponentParams()->get('actionlogs_integration', 0)) {
			$action = JText::_('COM_JMAP_ACTIONLOGS_COPIED');
			$this->storeActionLog($action, 'sources', JText::sprintf('COM_JMAP_ACTIONLOGS_DATA_SOURCE', $table->name), 'index.php?option=com_jmap&task=sources.editEntity&cid[]=' . $table->id, $table->id);
		}
		return true;
	}
	...
	public function deleteEntity($ids) {
		...
		// If integration with ActionLogs
		if(version_compare(JVERSION, '3.9', '>=') && $this->getComponentParams()->get('actionlogs_integration', 0)) {
			$action = JText::_('COM_JMAP_ACTIONLOGS_DELETED');
			$this->storeActionLog($action, 'sources', JText::sprintf('COM_JMAP_ACTIONLOGS_DATA_SOURCE', $table->name), 'javascript:void(0)', $id);
		}
	...
	public function publishEntities($idEntity, $state) {
		...
		if(version_compare(JVERSION, '3.9', '>=') && $this->getComponentParams()->get('actionlogs_integration', 0)) {
			...
			$action = $isPublished ? JText::_('COM_JMAP_ACTIONLOGS_PUBLISHED') : JText::_('COM_JMAP_ACTIONLOGS_UNPUBLISHED');
			$this->storeActionLog($action, 'sources', JText::sprintf('COM_JMAP_ACTIONLOGS_DATA_SOURCE', $table->name), 'index.php?option=com_jmap&task=sources.editEntity&cid[]=' . $table->id, $table->id);
		}
		return true;
	}
